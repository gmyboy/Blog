...
public class PublishActivity extends BaseActivity implements OnClickListener,
	...
	/** 获取at的人 */
	public static final int SELECT_AT = 4;
	/** 获取话题 */
	public static final int SELECT_TOPIC = 5;
	...
	private String atPer = "";// @的人
	private String topicPer = "";// #的话题
	private String back;
	private ViewPager pager_emo;// 表情页面
	private ImageView picture, emotion, text, camera, at, topic;// 底部洗个按钮
	...
	public void initView() {
		...
		at = (ImageView) this.findViewById(R.id.send_img05);// at
		topic = (ImageView) this.findViewById(R.id.send_img06);// topic
	...
	public void onClick(View v) {
			...
			if (edt_content.getText().toString().equals("")) {
				...
				new Thread() {
					public void run() {
						...
						Log.i("gmyboy", android.os.Build.MODEL);
		...
		case R.id.send_img04:
			...
			break;
		case R.id.send_img05:
			intent = new Intent();
			intent.setClass(mContext, AttentionUserActivity.class);
			intent.putExtra("FROM", TAG);
			startActivityForResult(intent, SELECT_AT);
			break;
		case R.id.send_img06:
			intent = new Intent();
			intent.setClass(mContext, TopicActivity.class);
			intent.putExtra("FROM", TAG);
			startActivityForResult(intent, SELECT_TOPIC);
			break;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if (resultCode == RESULT_OK) {
			...
			case SELECT_LOCATION:
				...
				break;
			case SELECT_AT:
				atPer = data.getStringExtra("name");
				if (!atPer.equals("")) {
					int start = edt_content.getSelectionStart();// 获取最后一个字符位置
					...
					if (info instanceof Spannable) {
						Spannable spanText = (Spannable) info;
						Selection.setSelection(spanText, start + atPer.length()
								+ 1);// 设置光标位置为spann之后
					}
				}
				break;
			case SELECT_TOPIC:
				topicPer = data.getStringExtra("name");
				if (!topicPer.equals("")) {
					int start = edt_content.getSelectionStart();// 获取最后一个字符位置
					edt_content.setText(edt_content.getText().toString() + "#"
							+ topicPer + "#");
					...
					if (info instanceof Spannable) {
						Spannable spanText = (Spannable) info;
						Selection.setSelection(spanText,
								start + topicPer.length() + 2);// 设置光标位置为spann之后
					}
				}
				break;
			}
